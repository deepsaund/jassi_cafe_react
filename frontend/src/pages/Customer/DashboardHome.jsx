...
export default function CustomerDashboard() {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            {/* Dark Premium Header */}
            <header className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-6 p-6 md:p-8 rounded-3xl relative overflow-hidden transition-all duration-500 ${theme === 'dark' ? 'bg-[#0f172a] text-white shadow-none' : 'bg-white text-slate-900 border border-slate-100 shadow-2xl shadow-blue-900/10'}`}>
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-2">
                        <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse shadow-glow shadow-blue-500/50" />
                        <span className="text-[9px] font-black uppercase tracking-widest text-blue-400">Identity Verified</span>
                    </div>
                    <h1 className="text-2xl md:text-3xl font-black tracking-tight flex items-center gap-3">
                        Welcome back, {user?.name?.split(' ')[0] || 'User'}!
                    </h1>
                    <p className="text-slate-400 mt-2 text-sm font-medium">Monitoring your active network and service requests.</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 relative z-10 w-full md:w-auto">
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-3xl p-4 flex items-center gap-4 backdrop-blur-md">
                        ...
                        <div>
                            ...
                            <p className="text-base font-black leading-none">₹{parseFloat(user?.wallet_balance || 0).toFixed(2)}</p>
                        </div>
                        <button
